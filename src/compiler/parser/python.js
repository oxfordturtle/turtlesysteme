...
code themselves are just stored for subsequent handling by the coder module.

The function is more or less a finite state machine, looping through the lexemes and behaving
differently according to its current state. The behaviour at each state is separated into a
different function, for readability, each of which return the index of the next lexeme and the next
machine state. One such function (def) also returns a reference to the newly created routine. All of
these functions have side-effects, modifying the routine currently being dealt with.

Note: New routines are created by the def function, and placed on top of the subroutine stack. When
the routine commands have all been dealt with, the routine is popped off the stack, and pushed to
the routines array (which is what the function ultimately returns). In the meantime, any subroutines
for that routine may have been pushed, handled, and popped. This ensures that the output routines
