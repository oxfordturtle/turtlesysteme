...
define(function (require) {
    ...
    var pc = require("data/pc");
    var commands = require("./commands");
    var find = require("./find");
    var values = require("./values");

    var newError;
    var newBlockEndError;
    var elseCheck;
    var compileIf;
    var compileFor;
    var compileRepeat;
    ...
    // ----------
    newError = function (errorId, messageId, lexeme) {
        var messages;
        messages = {
            ...
            forNotInteger: "'" + lexeme.string + "' is not an integer "
                    + "variable.",
            forAssignment: "'FOR' variable must be assigned an initial value.",
            forEquals: "Assignment operator is ':=', not '='.",
            forInitial: "'FOR' loop variable must be assigned an initial "
                    + "value.",
            forToDownTo: "'FOR ... := ...' must be followed by 'TO' or "
                    + "'DOWNTO'.",
            forToNothing: "'" + lexeme.string.toUpperCase() + "' must be "
                    + "followed by an integer (or integer constant).",
            ...
            repeatExpression: "'UNTIL' must be followed by a boolean "
                    + "expression.",
            whileExpression: "'WHILE' must be followed by a boolean "
                    + "expression.",
            ...
            cmdEqalPascal: "Variable assignment requires ':='rather than '='.",
            cmdEqalPython: "Variable assignment requires '='rather than '=='.",
    ...
    // ----------
    newBlockEndError = function (start, lexeme) {
        switch (start) {
        case "begin":
    ...
    // check if the current lexeme is "else" (and on the right line for BASIC)
    // ----------
    elseCheck = function (language, lexemes, lex, oneLine) {
        if (!lexemes[lex]) {
            return false;
        }
